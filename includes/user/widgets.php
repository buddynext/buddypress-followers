...
/**
 * Add a "My Followers" widget for the logged-in user
 *
 * @subpackage Widgets
 */
class BP_Follow_Followers_Widget extends WP_Widget {
	...
	function __construct() {
		// Set up optional widget args.
		$widget_ops = array(
			'classname'   => 'widget_bp_follow_followers_widget widget buddypress',
		...
		parent::__construct(
			false,
			__( "(BP Follow) My Followers", 'buddypress-followers' ),
			$widget_ops
		);
	}
	...
	function widget( $args, $instance ) {
		...
		// logged-in user has no followers, so stop!
		if ( ! $followers = bp_get_follower_ids( array( 'user_id' => bp_loggedin_user_id() ) ) ) {
			return false;
		}
	...
	?>
		...
		<p><label for="bp-follow-widget-users-max"><?php _e('Max members to show:', 'buddypress-followers'); ?> <input class="widefat" id="<?php echo $this->get_field_id( 'max_users' ); ?>" name="<?php echo $this->get_field_name( 'max_users' ); ?>" type="text" value="<?php echo esc_attr( (int) $instance['max_users'] ); ?>" style="width: 30%" /></label></p>
		<p><small><?php _e( 'Note: This widget is only displayed if a member is logged in and if the logged-in user has followers.', 'buddypress-followers' ); ?></small></p>
