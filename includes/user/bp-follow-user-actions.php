...
function bp_follow_action_start() {
	...
	$is_follow_component = bp_is_current_component( $bp->follow->followers->slug ) || bp_is_current_component( $bp->follow->following->slug );

	if ( ! $is_follow_component || ! bp_is_current_action( 'start' ) ) {
		return;
	}
...
function bp_follow_action_stop() {
	...
	if ( ! $is_follow_component || ! bp_is_current_action( 'stop' ) ) {
		return;
	}
