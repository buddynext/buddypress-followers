...
function bp_follow_load_template_filter( $found_template, $templates ) {
	...
	if ( empty( $found_template ) ) {
		...
		// For BP Nouveau, also hook into the plugin content action
		if ( function_exists( 'bp_nouveau' ) ) {
			add_action( 'bp_nouveau_plugin_hook_content', function() {
				bp_get_template_part( 'members/single/follow' );
			} );
		}
	}
...
function bp_follow_get_template_directory() {
	return apply_filters( 'bp_follow_get_template_directory', constant( 'BP_FOLLOW_DIR' ) . '/templates' );
}
