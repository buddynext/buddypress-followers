...
function bp_follow_user_setup_nav( $main_nav = array(), $sub_nav = array() ) {
	...
	// Register the default subnav item for following
	bp_core_new_subnav_item(
		array(
			...
			'screen_function' => 'bp_follow_screen_following',
			'position'        => 10,
			'item_css_id'     => 'following-following',
		)
	);
	...
	// Register the default subnav item for followers
	bp_core_new_subnav_item(
		array(
			'name'            => __( 'Followers', 'buddypress-followers' ),
			'slug'            => 'followers',
			'parent_url'      => bp_follow_get_user_url( $user_id, array( $bp->follow->followers->slug ) ),
			'parent_slug'     => $bp->follow->followers->slug,
			'screen_function' => 'bp_follow_screen_followers',
			'position'        => 10,
			'item_css_id'     => 'followers-followers',
		)
	);
...
/**
 * Add proper loop classes for BP Nouveau follow pages.
 *
 * Makes follow pages render with the same grid/list layout as friends pages.
 *
 * @since 2.0.0
 *
 ...
 */
function bp_follow_nouveau_loop_classes( $classes, $component ) {
	if ( bp_is_user() && in_array( bp_current_action(), array( 'following', 'followers' ), true ) ) {
		$classes[] = 'members-list';

		// Use members component settings for grid layout
		// This makes follow pages use the same layout as friends/members
		$layout_prefs = bp_nouveau_get_temporary_setting(
			'members_friends_layout',
			bp_nouveau_get_appearance_settings( 'members_friends_layout' )
		);
		...
		if ( $layout_prefs && (int) $layout_prefs > 1 ) {
			...
			if ( isset( $grid_classes[ $layout_prefs ] ) ) {
				$classes[] = 'grid';
				$classes[] = $grid_classes[ $layout_prefs ];
			}
		}
	}
	return $classes;
}
add_filter( 'bp_nouveau_get_loop_classes', 'bp_follow_nouveau_loop_classes', 10, 2 );
 ...
 * available in /includes/compatibility/activity-scope.php.
 *
 * @since 1.3.0
 *
...
function bp_follow_add_member_scope_filter( $qs, $object ) {
	...
	switch ( $action ) {
		...
		case 'followers':
			$r['include'] = bp_follow_get_followers( array(
				'user_id' => bp_displayed_user_id() ? bp_displayed_user_id() : bp_loggedin_user_id(),
			) );
