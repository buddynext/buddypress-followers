...
jq( function() {
	...
	if ( jq( 'body.bp-nouveau' ).length ) {
		...
		// Support both directory and profile tabs (including AJAX-loaded content)
		// Use the container that exists on page load for event delegation
		if ( jq('[data-bp-list="members"]').length ) {
			memberLoop = jq('[data-bp-list="members"]');
		} else {
			memberLoop = jq("#members-dir-list, #members-list").parent();
		}
